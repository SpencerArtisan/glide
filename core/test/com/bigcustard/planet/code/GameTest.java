...
public class GameTest {
    ...
    public void hasNoRecentWhenInPrefsButNoDirectory() {
        when(files.local("games/planet").exists()).thenReturn(false);
        when(preferences.getString("MostRecentGameName")).thenReturn("planet");
        assertThat(Game.hasMostRecent(preferences, files)).isFalse();
    }
    ...
    public void hasNoRecentWhenInPrefsButNoCodeFile() {
        when(preferences.getString("MostRecentGameName")).thenReturn("planet");
        when(files.local("games/planet").exists()).thenReturn(true);
        when(files.local("games/planet/code.groovy").exists()).thenReturn(false);
        assertThat(Game.hasMostRecent(preferences, files)).isFalse();
    }
    ...
    public void hasRecentWhenInPrefsAndDirectoryAndCodeFileExist() {
        when(preferences.getString("MostRecentGameName")).thenReturn("planet");
        when(files.local("games/planet").exists()).thenReturn(true);
        when(files.local("games/planet/code.groovy").exists()).thenReturn(true);
    ...
    public void mostRecentLoadsCode() {
        ...
        when(files.local("games/planet").exists()).thenReturn(true);
        when(files.local("games/planet/code.groovy").exists()).thenReturn(true);
        when(files.local("games/planet/code.groovy").readString()).thenReturn("code");
        when(files.local("games/planet/manifest.json").readString()).thenReturn("{}");
    ...
    public void mostRecentLoadsImages() {
        ...
        when(files.local("games/planet").exists()).thenReturn(true);
        when(files.local("games/planet/code.groovy").exists()).thenReturn(true);
        when(files.local("games/planet/manifest.json").readString()).thenReturn("{images:[{filename:image.png,name:image,width:100,height:50}]}");
